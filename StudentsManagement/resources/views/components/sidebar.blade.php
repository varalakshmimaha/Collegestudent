...
<div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-950 border-r border-white/5 transition duration-300 transform lg:translate-x-0 lg:static lg:inset-0 flex flex-col h-full overflow-y-auto">
    ...
    <div class="flex flex-col flex-1 overflow-y-auto custom-scrollbar">
        <nav class="flex-1 px-4 py-6 space-y-1">
            ...
            @if((auth()->user()->role && auth()->user()->role->name === 'Super Admin') || auth()->user()->hasPermissionTo('view colleges'))
                <a href="{{ route('colleges.index') }}" class="flex items-center px-4 py-3 text-sm font-bold rounded-xl transition {{ request()->routeIs('colleges.*') ? 'bg-white text-black shadow-lg shadow-white/5' : 'text-zinc-300 hover:bg-white/10 hover:text-white' }}">
                    <svg class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if((auth()->user()->role && auth()->user()->role->name === 'Super Admin') || auth()->user()->hasPermissionTo('view branches'))
            ...
            @if((auth()->user()->role && auth()->user()->role->name === 'Super Admin') || auth()->user()->hasPermissionTo('view lead statuses'))
            ...
            <div class="pt-6 pb-2 px-4">
                <p class="text-[10px] font-black text-white uppercase tracking-widest">Financial</p>
            </div>
